...
KEYWORDS = [
    "압구정쌍수","눈성형","눈수술","눈추천","자연유착","자연유착쌍수","매몰법","절개법","쌍꺼풀","쌍꺼풀수술",
    ...
    "쌍수","자연유착쌍꺼풀","쌍꺼풀매몰법","쌍꺼풀절개법","쌍수붓기","쌍수후기","쌍꺼풀라인","재수술붓기",
    "쌍수 전후","쌍수 비용","쌍수 가격","쌍수 회복기간","쌍수 부작용","쌍수 브이로그","쌍꺼풀 재수술 후기","쌍꺼풀 재수술 비용","쌍꺼풀 재수술 시기","소세지눈 재수술","쌍꺼풀 라인 낮추기","쌍꺼풀 비대칭 재수술","쌍꺼풀 풀림 재수술","자연유착 후기","자연유착 붓기","자연유착 매몰 차이","자연유착 절개 차이","자연유착 풀림","매몰법 후기","매몰법 붓기","매몰법 풀림","매몰 절개 차이","절개 쌍꺼풀 후기","절개 쌍꺼풀 붓기","절개 쌍꺼풀 흉터","절개 쌍꺼풀 회복기간","눈매교정 후기","눈매교정 부작용","눈매교정 필요한 눈","눈매교정 안검하수","비절개 눈매교정","쌍수 붓기 빠지는 과정","쌍수 붓기 빨리 빼는법","쌍수 한달 후기","쌍꺼풀 라인 종류","인아웃라인 쌍꺼풀","세미아웃라인 쌍꺼풀","인아웃라인 세미아웃 차이","자연스러운 쌍꺼풀","쌍꺼풀 라인 추천","쌍꺼풀 수술 부작용"
]
...
def main():
    ...
    for kw in KEYWORDS:
        ...
        try:
            ...
            comp[kw] = [{"t": x["title"], "c": x["channelTitle"], "m": is_mine(x), "d": (x.get("description") or "")[:160], "v": x.get("videoId", "")} for x in clinic[:10]]
...
.vid .sm{color:var(--faint);font-size:11px;margin-top:3px;line-height:1.45}
.vid .tt a:hover{text-decoration:underline!important}
...
<div class="view" id="view-content">
  ...
  <div id="comp"></div>
  <h2>🎬 키워드별 추천 영상 (성형외과 · 제목+요약)</h2>
  <p class="note">성형외과 채널이 올린 상위 영상이에요. 제목을 클릭하면 유튜브로 바로 이동합니다. (초록색 = 우리 채널)</p>
  <div id="realvids"></div>
</div>
...
document.getElementById('comp').innerHTML=compKws.length?compKws.map(function(kw){var vids=(COMPD[kw]||[]).slice(0,3),r=rankOf(kw),patt={};vids.forEach(function(v){PATTERNS.forEach(function(p){if(p.re.test(v.t))patt[p.label]=1;});});var off=OFF.has(kw);return '<div class="compkw"><div class="h">'+kw+' <span class="tag '+(r?'ok':'beyond')+'">'+(r?'우리 '+r+'위':'우리 미진입')+'</span>'+(off?' <span class="tag off">의도불일치</span>':'')+'</div>'+vids.map(function(v,i){return '<div class="vid '+(v.m?'mine':'')+'"><span class="n">'+(i+1)+'</span><div><div class="tt">'+v.t+(v.m?' ✅':'')+'</div><div class="cc">'+v.c+'</div></div></div>';}).join('')+(Object.keys(patt).length?'<div class="pat">'+Object.keys(patt).map(function(p){return '<span class="p">#'+p+'</span>';}).join('')+'</div>':'')+'</div>';}).join(''):'<div class="note">이 날짜엔 경쟁사 스냅샷이 없어요. (수집기 보강 후 날짜부터 날짜별로 쌓여요.)</div>';
document.getElementById('realvids').innerHTML=compKws.length?compKws.map(function(kw){var vids=COMPD[kw]||[],r=rankOf(kw),off=OFF.has(kw);return '<div class="compkw"><div class="h">'+kw+' <span class="tag '+(r?'ok':'beyond')+'">'+(r?'우리 '+r+'위':'우리 미진입')+'</span>'+(off?' <span class="tag off">의도불일치</span>':'')+'</div>'+vids.map(function(v,i){var link=v.v?'https://www.youtube.com/watch?v='+v.v:'';var title=link?'<a href="'+link+'" target="_blank" rel="noopener" style="color:inherit;text-decoration:none">'+v.t+'</a>':v.t;var summ=v.d?'<div class="sm">'+v.d+'</div>':'';return '<div class="vid '+(v.m?'mine':'')+'"><span class="n">'+(i+1)+'</span><div style="flex:1"><div class="tt">'+title+(v.m?' ✅':'')+'</div><div class="cc">'+v.c+'</div>'+summ+'</div></div>';}).join('')+'</div>';}).join(''):'<div class="note">이 날짜엔 성형외과 영상 스냅샷이 없어요.</div>';
}
}
</script></body></html>"""
