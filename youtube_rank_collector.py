...
KEYWORDS = [
    "눈성형","눈수술","12홀자연유착","자연유착","매몰법","절개법","쌍꺼풀","쌍꺼풀수술",
    "눈재수술","인라인","인아웃라인","세미아웃라인","아웃라인","앞트임","위트임","뒤트임",
    "밑트임","상안검","하안검","눈매교정","눈밑지방재배치","눈썹하거상","안면거상","이마거상",
    "미니거상","거상","실리프팅","리프팅","이중턱","남자눈성형","코수술","코성형",
]
...
def main():
    ...
    for kw in KEYWORDS:
        ...
        try:
            items = yt_search(kw)
            comp[kw] = [{"t": x["title"], "c": x["channelTitle"], "m": is_mine(x)} for x in items[:3]]
...
<div class="sub">채널 <b id="ch"></b> · 쇼츠 상위노출 기준 · 최신 수집일 <b id="ld"></b> · 추적 키워드 <b id="nk"></b>개 · 생성 <span id="gen"></span></div>
<div class="tabs">
...
document.getElementById('actions').innerHTML=acts.slice(0,6).map(function(a,i){return '<div class="act"><div class="rn">'+(i+1)+'</div><div><div class="at">'+a.t+'</div><div class="ad">'+a.d+'</div></div></div>';}).join('')||'<div class="note">아직 TOP50 진입 키워드가 없어요. 데이터가 쌓이면 우선 액션이 표시됩니다.</div>';
...
var compKws=Object.keys(COMP).filter(function(k){return COMP[k]&&COMP[k].length;});
document.getElementById('comp').innerHTML=compKws.map(function(kw){var vids=COMP[kw],r=rankOf(kw),patt={};vids.forEach(function(v){PATTERNS.forEach(function(p){if(p.re.test(v.t))patt[p.label]=1;});});var off=OFF.has(kw);return '<div class="compkw"><div class="h">'+kw+' <span class="tag '+(r?'ok':'beyond')+'">'+(r?'우리 '+r+'위':'우리 미진입')+'</span>'+(off?' <span class="tag off">의도불일치</span>':'')+'</div>'+vids.map(function(v,i){return '<div class="vid '+(v.m?'mine':'')+'"><span class="n">'+(i+1)+'</span><div><div class="tt">'+v.t+(v.m?' ✅':'')+'</div><div class="cc">'+v.c+'</div></div></div>';}).join('')+(Object.keys(patt).length?'<div class="pat">'+Object.keys(patt).map(function(p){return '<span class="p">#'+p+'</span>';}).join('')+'</div>':'')+'</div>';}).join('');
}
</script></body></html>"""
